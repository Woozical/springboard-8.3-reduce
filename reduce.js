...
function vowelCount(str) {
    ...
    const vowelsOnly = Array.from(str).filter(
        function(char){
            return 'aeiou'.indexOf(char) !== -1;
        }
    )

    return vowelsOnly.reduce(
        function(result, char){
            const caseIn = char.toLowerCase();
            if (result[caseIn]){
                result[caseIn] += 1;
            } else {
                result[caseIn] = 1;
...
function partition(arr, callback) {
    ...
    return arr.reduce(
        function(resultArray, elem){
            ...
            if (callback(elem)){
                resultArray[0].push(elem);
            } else {
                resultArray[1].push(elem);
            }
            return resultArray;
        }, [ [],[] ]
    );
